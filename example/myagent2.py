...
'''
    ...
    def my_product_summary(self, product):
        product_info = {key: product[key] for key in ['item_id', 'name','stars','review_count','attributes','title', 'average_rating', 'rating_number','description','ratings_count','title_without_series','details','main_category', 'categories'] if key in product}
        prompt = f'''
...
'''
    ...
    def workflow(self):
        ...
        if len(user_reviews) > 0:
            ...
            user_review_product_adj_score = [self.my_rating(score=review['stars'],
                                                        # additional_indicator=0,
                                                        min_score=min_user_review_stars,
                                                        max_score=max_user_review_stars,
                                                        avg_score=user_avg_stars) for review in user_reviews]
            ...
            user_review_infos = [{'review_text': review['text'],
                                  'score': adj_score,
                                  'product_info': product_summary} for review, adj_score, product_summary in zip(user_reviews, user_review_product_adj_score, user_review_product_summaries)]
        ...
        if len(user_friends) > 0:
            ...
            for friend_id in user_friends:
                ...
                friend_review_product_adj_score = [self.my_rating(score=review['stars'],
                                                            # additional_indicator=0,
                                                            min_score=min_friend_review_stars,
                                                            max_score=max_friend_review_stars,
                                                            avg_score=friend_avg_stars) for review in friend_reviews]
                friend_review_products = [self.interaction_tool.get_item(item_id=item_id) for item_id in friend_review_product_list]
                friend_review_product_summaries = [self.my_product_summary(product=product) for product in friend_review_products]
                friend_review_infos = [{'review_text': review['text'],
                                      'score': adj_score,
                                      'product_info': product_summary} for review, adj_score, product_summary in zip(friend_reviews, friend_review_product_adj_score, friend_review_product_summaries)]
...
'''
        ...
        for i in range(len(current_candidate_list)):
            candidate_info_str = f"Item ID: {current_candidate_list[i]}\nSummary: {candidate_product_summaries[i]}\n"
            candidate_short_info = {key: candidate_dicts[i][key] for key in ['item_id', 'name','stars','review_count','attributes','title', 'average_rating', 'rating_number','description','ratings_count','title_without_series','details','main_category', 'categories'] if key in candidate_dicts[i]}
            candidate_info_str += "\n".join([f"{key}: {value}" for key, value in candidate_short_info.items()])
...
'''
        ...
        try:
            ...
            print('Processed Output:',eval(final_list))
            ...
            # This catches errors if the string is not a valid list format
